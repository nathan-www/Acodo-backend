...
class Session{
  ...
  public static function sessionMiddleware(Request $request, RequestHandler $handler){
    ...
    if(!isset($_COOKIE['acodo_session_id']) || !isset($_COOKIE['acodo_session_token'])){
      throw new HttpUnauthorizedException($request, "Missing session credentials");
    }

    $session = self::db()->select('sessions',['session_id'=>$_COOKIE['acodo_session_id'],'token'=>$_COOKIE['acodo_session_token']]);
    if(count($session) < 1){
        throw new HttpUnauthorizedException($request, "Invalid session credentials");
    }

    //Update activity in session  database
    self::db()->update('sessions',['session_id'=>$_COOKIE['acodo_session_id']],["last_activity"=>time()]);

    return $handler->handle($request->withAttribute('user_id',$session[0]['user_id'])->withAttribute('session_id',$_COOKIE['acodo_session_id']));
  }
